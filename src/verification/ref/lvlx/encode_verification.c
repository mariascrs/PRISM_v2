...
signature_to_bytes(byte_t *enc, const signature_t *sig)
{
    ...
    enc = ec_curve_to_bytes(enc, &sig->E_rsp);

    // Pack the FULL 4-byte counter
    memcpy(enc, &sig->counter, sizeof(uint32_t));
    enc += sizeof(uint32_t); // Move the pointer forward 4 bytes
    
    enc = ec_basis_to_bytes(enc, &sig->B_rsp);
     
#ifndef NDEBUG
    // TODO: fix in general
    printf("----------------------------------------------------------------------------------------------\n");
    printf("Hack to set the correct SIGNATURE_BYTES.\n");
    printf("SIGNATURE_BYTES manually changed to %ld. Also changed CRYPTO_BYTES to match.\n", enc - start);
...
signature_from_bytes(signature_t *sig, const byte_t *enc)
{
    ...
    enc = ec_curve_from_bytes(&sig->E_rsp, enc);

    // Recover the FULL 4-byte counter
    // We copy 4 bytes from the buffer into the sig->counter variable
    memcpy(&sig->counter, enc, sizeof(uint32_t));
    enc += sizeof(uint32_t);

    enc = ec_basis_from_bytes(&sig->B_rsp, enc);
